...
interface AchievementBlockProps {
    ...
    colorClass: string; // e.g., 'pink', 'purple', 'green'
}

const AchievementBlock: React.FC<AchievementBlockProps> = ({ icon, title, content, colorClass }) => {
    
    // Dynamic Tailwind classes for gradient, borders, text, and hover effects
    const bgColor = `bg-gradient-to-br from-${colorClass}-50 to-${colorClass}-100`;
    const borderColor = `border-${colorClass}-600`;
    const iconColor = `text-${colorClass}-600`;
    const titleColor = `text-${colorClass}-800`; // Dynamically color the title
    const contentColor = `text-${colorClass}-700`; // Dynamically color the content
    const hoverShadow = `shadow-${colorClass}-400/60`; // Pronounced colored shadow
    const ringHover = `ring-${colorClass}-200`; // Light ring color on hover

    return (
        <div 
            className={`
                // Base Styling: Gradient background, rounded, lift
                ${bgColor} p-6 rounded-xl shadow-lg 
    ...
    return (
        <div className="min-h-screen py-16 px-4 sm:px-6 lg:px-8 bg-transparent">
            <div className="max-w-6xl mx-auto">
                ...
                {/* Main Heading (White contrast against video background) */}
                ...
                {/* INTRODUCTION BLOCK (Enhanced Gradient) */}
                ...
                <div className="grid md:grid-cols-2 gap-8">
                    ...
                    {/* Card 1: Global Experience - Pink Theme */}
                    <AchievementBlock
                    ...
                    {/* Card 2: Academic & Corporate Roots - Purple Theme */}
                    <AchievementBlock
                        icon={<Award size={24} />}
                    ...
                    {/* Card 3: Entrepreneurial Success - Green Theme */}
                    <AchievementBlock
                        icon={<Zap size={24} />}
                    ...
                    {/* Card 4: Driving Philosophy - Highly styled static block (Teal/Indigo Theme) */}
                    <div className="p-6 bg-gradient-to-br from-indigo-50 to-teal-100 rounded-xl shadow-xl border-r-4 border-teal-600 flex flex-col justify-center transition-all duration-300 hover:shadow-teal-400/50 hover:scale-[1.03] hover:translate-y-[-2px] cursor-pointer">
                        <div className="flex items-center mb-3">
                            <Anchor size={28} className="mr-3 text-teal-700" />
                            <h3 className="text-xl font-extrabold text-teal-800">Core Mission: The Anchor</h3>
                        </div>
